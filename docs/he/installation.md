...
### שלב 4.5 — (אופציונלי) בחירת backend לאחסון מסמכים גדולים

אחסון המסמכים תומך ב-**ארבעה backends** — אפשר לבחור בהגדרות → אחסון מסמכים לאחר הכניסה הראשונה. מסמכים קיימים נשארים תמיד אצל הספק שאליו הועלו, אז ההחלפה בטוחה.
...
| **Cloudflare R2** | 10GB/חודש + ללא עלויות הורדה | bucket + הפעלת R2 בלוח הבקרה | עד 20MB | דורש אמצעי תשלום (גם אם הצריכה $0). מתאים לקבצים גדולים יותר במי שכבר מחזיק חשבון Cloudflare. |
| **Backblaze B2** | 10GB/חודש | הרשמה + Application Key, מוגדר ב-UI | עד 20MB | **ללא צורך בכרטיס אשראי לתוכנית החינמית.** מומלץ למי שרוצה object storage בלי לתת כרטיס ל-Cloudflare. |
| **Google Drive** | 15GB לחשבון | OAuth (שלב 6 + שלב 9) | עד 20MB | תלוי בכך שחשבון Google ישאר פעיל. |

**ברירת המחדל היא D1** — עובד ברגע שה-schema הוחל, ללא שלבים נוספים. ההגדרה של האחרים מתבצעת לאחר הכניסה הראשונה:

**R2 (אם רוצים):**
```bash
npx wrangler r2 bucket create vaad-docs
```
אם מקבלים `Please enable R2 through the Cloudflare Dashboard`: היכנס ל-https://dash.cloudflare.com → R2 Object Storage → לחץ **Purchase R2** (דורש אמצעי תשלום, גם אם הצריכה $0). הסר את ההערה מהבלוק `[[r2_buckets]]` ב-`wrangler.toml` ופרוס מחדש.

**Backblaze B2 (אם רוצים):**
1. הירשם ב-https://www.backblaze.com/cloud-storage (לא דורש כרטיס לתוכנית החינמית).
2. צור bucket פרטי — שים לב לשם המדויק.
3. App Keys → "Add a New Application Key" עם הרשאת קריאה+כתיבה ל-bucket הזה. העתק את ה-**keyID** וה-**applicationKey** (המפתח מוצג פעם אחת בלבד).
4. לאחר הכניסה הראשונה: הגדרות → אחסון מסמכים → dropdown → **Backblaze B2** → הדבק את ה-keyID, applicationKey, שם ה-bucket, ו-(אופציונלי) ה-endpoint שמופיע ליד ה-bucket ב-Backblaze. לחץ **שמור והגדר** — השרת יבצע round trip אמיתי מול B2 וידחה credentials שגויים לפני השמירה.
5. לחץ **הפעל ספק זה** כדי לעשות את B2 ה-backend הפעיל להעלאות חדשות.

אם לא מגדירים אף אחד מ-R2/B2/Drive, D1 ממשיך לטפל בהכל — זה הנתיב המומלץ ל-zero-config.

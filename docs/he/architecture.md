...
│  (SQLite)        │ │ טבלה:      ││ bucket:    ││ (חשבון המנהל)    │
│  • טבלאות אפ׳    │ │ document_  ││ vaad-docs  ││ תיקייה: vaad-docs│
│  • sessions      │ │ d1_blobs   ││            ││                  │
│  • audit log     │ │            ││ binding:   ││ OAuth: drive.file│
│  • PBKDF2 hashes │ │ ברירת מחדל ││ DOCS_      ││                  │
│  • סודות         │ │ ללא הגדרה  ││ BUCKET     ││ טוקן מוצפן ב-D1  │
│    מוצפנים       │ │ עד ~5MB    ││ 10GB חינם, ││                  │
│                  │ │ פר קובץ    ││ ללא egress ││                  │
└──────────────────┘ └────────────┘└────────────┘└──────────────────┘
```

**אחסון מסמכים** הוא **שלושה backends**. העלאות חדשות הולכות לספק שהמנהל בחר (ברירת מחדל: **D1** — ללא צורך בהגדרה ובלי כרטיס אשראי). כל שורה בטבלת `documents` משויכת לשורה ב-`document_storage` (או ל-`document_d1_blobs` עבור D1) שמציינת באיזה backend הקובץ נמצא — כך שהורדות ומחיקות מנותבות למקום הנכון בלי תלות באיזה ספק היה פעיל בזמן ההעלאה. החלפת ספקים בהגדרות משפיעה רק על העלאות *עתידיות*; מסמכים ישנים נשארים היכן שהם.

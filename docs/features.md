...
- **Multi-phone per owner** — each owner can register any number of phone numbers, each with an optional label (e.g. "spouse", "son"). The first phone is the primary and shows in the apartments-and-charges table; the owner edit dialog has a dynamic add/remove list. Stored in a dedicated `owner_phones` table (idempotent additive migration).

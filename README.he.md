...
- **בעלים / שוכר** — כל דירה מסומנת כ"בעלים גר בדירה" או "שוכר". כשמדובר בשוכר, רושמים את פרטי השוכר *ו*בנפרד את פרטי בעלי הדירה (שם, טלפון, אימייל).
...
- **כניסה עם Google** — דיירים (שוכרים ובעלים) יכולים לבחור להיכנס עם חשבון Google שלהם במקום סיסמה. המנהל מגדיר את ה-Gmail של המשתמש בעת יצירתו; המשתמש לוחץ "התחבר עם Google" במסך הכניסה ונכנס ישירות. משתמש באותו OAuth client של שחזור הסיסמה — אין הגדרות נוספות.
...
### שחזור סיסמה (מנהל ראשי וכל דירה)

מערכת השחזור היא **פר-משתמש** — לא משותפת:

- **המנהל הראשי** רושם את חשבון Google שלו.
- **כל דירה** (דייר רגיל *או* דייר עם הרשאת אדמין) רושמת חשבון משלה — חשבון Google שונה מזה של המנהל הראשי, ושונה מכל דירה אחרת.
- דיירים עם הרשאת אדמין **אינם** "אותו משתמש" כמו המנהל הראשי. הם דיירים רגילים עם הרשאת ניהול; הזהות שלהם היא הדירה, והשחזור שלהם הוא פר-דירה.
...
**דרישות** — שחזור עובד לכל חשבון שרשם כתובת לאיפוס:
- המנהל הראשי או הדייר כבר אימת חשבון Google ב-הגדרות → אבטחה → אימות זהות.
- ה-OAuth client של Google מוגדר (כבר משלב 6).

**זה הכל.** בלי Resend. בלי דומיין מאומת. בלי תשתית מייל יוצא.

**זרימה — מנהל ראשי:**
1. מסך כניסה, לשונית **מנהל**.
2. לחץ **"שכחתי סיסמה?"** מתחת לשדה הסיסמה.
3. במודאל לחץ **"התחבר עם Google"**.
4. התחבר עם חשבון השחזור משלב 8.5.
5. בהתאמה → טופס סיסמה חדשה. אי-התאמה → שגיאה מנומסת.
6. בחר סיסמה חדשה ושמור.

**זרימה — דירה (דייר רגיל או דייר עם הרשאת אדמין):**
1. מסך כניסה, לשונית **דייר**.
2. בחר את הדירה ב-dropdown והקלד בשדה הסיסמה — קישור **"שכחתי סיסמה?"** יופיע מתחתיו.
3. לחץ עליו, ואז **"התחבר עם Google"**.
4. התחבר עם חשבון Google שנרשם לדירה הזו (הגדרות → אבטחה → אימות זהות, כשאתה מחובר כדירה ההיא).
5. בהתאמה → טופס סיסמה חדשה **לדירה הזו בלבד**. אי-התאמה → שגיאה מנומסת.

**השפעות בעקבות שחזור מוצלח:**
- שחזור מנהל ראשי: כל הסשנים של המנהל הראשי מנותקים; **2FA מבוטל אוטומטית** (סיבה: מי ששכח סיסמה לרוב איבד גם את ה-authenticator — כיבוי 2FA מונע נעילה לצמיתות; ניתן להפעיל מחדש בהגדרות).
- שחזור דירה: רק הסשנים של הדירה הזו מנותקים. משתמשים אחרים (מנהל ראשי, דירות אחרות) לא מושפעים.

**החלפת חשבון לאיפוס:**
הגדרות → אבטחה → "החלפת חשבון לאיפוס" מריץ OAuth עם `purpose=replace`. הכתובת הנוכחית מוחלפת **רק לאחר אימות מוצלח של חשבון Google החדש**. זמין כשמחוברים כמשתמש שאת החשבון שלו מחליפים (כך שדייר עם הרשאת אדמין מחליף את חשבון השחזור של הדירה שלו, לא של המנהל הראשי).

**מניעת ניצול:**
ה-endpoint של reset מוגבל לפי IP (5 בקשות / 5 דקות). חשבון Google לא תואם נרשם ב-audit log תחת `identity_reset_mismatch` כדי שניסיונות יהיו גלויים. אותה שגיאה גנרית מוחזרת בין אם הדירה קיימת, אם רשום לה חשבון שחזור, או אם הייתה התאמה — כדי לא לחשוף אילו דירות ניתנות לשחזור.

**אם שחזור המנהל הראשי לא זמין:**
תיכנס למצב הזה רק אם מחקת את חשבון השחזור (או מעולם לא רשמת אחד). חלופה: לאפס את סיסמת המנהל הראשי ישירות ב-D1:
...
המנהל הראשי יכול לאפס את סיסמת כל דירה מעמוד ניהול הדירות — הדירה תיכנס מחדש לתהליך "התקנה ראשונה" בכניסה הבאה.

### למה דיירים עם הרשאת אדמין הם **לא** "אותו משתמש" כמו המנהל הראשי

זה הכרחי להבהיר כי גרסאות קודמות של המערכת טשטשו את הקו. כיום:

- ל**מנהל הראשי** יש כניסה משלו (לשונית "מנהל" הגלובלית) שעובדת מול שורת `admin_auth` יחידה.
- **דיירים עם הרשאת אדמין** הם פשוט דיירים (כניסה דרך לשונית "דייר") שלדירתם ניתנה הרשאת אדמין דרך טבלת `apartment_admins`. יש להם הרשאות ניהול ב-UI, אבל **הזהות שלהם היא הדירה**: הסיסמה שלהם נמצאת ב-`apartments.password_hash`, חשבון השחזור שלהם ב-`apartment_recovery`, וה-session שלהם נושא `apartmentId`.
- דייר עם הרשאת אדמין **לא יכול** להחליף את סיסמת המנהל הראשי (ה-endpoint של `change-password` אוכף את זה — `kind=admin` דורש session ללא `apartmentId`).
- המנהל הראשי **כן יכול** לאפס את סיסמת כל דירה דרך עמוד ניהול הדירות (במכוון — הוא הבעלים של היררכיית החשבונות בבניין).
